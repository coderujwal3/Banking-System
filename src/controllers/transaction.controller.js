...
async function createTransaction(req, res) {
    ...
    const fromAccountExist = await accountModel.findOne({ _id: fromAccount }).populate('user', 'phone name email');
    const toAccountExist = await accountModel.findOne({ _id: toAccount }).populate('user', 'phone name email');
    ...
    try {
        ...
        session.endSession();
        // When the transaction fails, send the transaction failure message to fromUser
        await MessageService.sendTransactionFailure(fromAccountExist.user.phone, fromAccountExist.user.name, amount, fromAccount, toAccount);
        return res.status(403).json({
            message: "Transaction was not committed, try again later",
            status: "failed"
        })
    }
    ...
    // send confirmation in email also
    await emailService.sendTransactionCompletedEmail_fromUser(fromAccountExist.user.email, fromAccountExist.user.name, amount, fromAccount, toAccount)
    ...
    // send credited amount mail in receiver's email id
    await emailService.sendTransactionCompletedEmail_toUser(toAccountExist.user.email, toAccountExist.user.name, amount, fromAccount, toAccount)
